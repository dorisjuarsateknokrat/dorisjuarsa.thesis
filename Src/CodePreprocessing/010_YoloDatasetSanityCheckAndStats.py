...
def parse_names_from_yaml(yaml_path: Path) -> Dict[int, str]:
    """
    Parse class names dari data.yaml (format mapping seperti dataset Anda):
    ...
    Tidak hardcode. Jika tidak ketemu / invalid -> error.
    """
    if not yaml_path.exists():
        raise FileNotFoundError(f"[ERROR] data.yaml tidak ditemukan: {yaml_path}")
    ...
    for line in lines:
        ...
        if in_names:
            # kalau ketemu top-level key lain, stop parse names
            ...
            if not k.isdigit():
                raise ValueError(f"[ERROR] index class bukan integer: '{k}' (file: {yaml_path})")

            names[int(k)] = v

    if not names:
        raise ValueError(f"[ERROR] 'names:' tidak ditemukan / kosong di data.yaml: {yaml_path}")

    # Optional: validasi urutan 0..N-1 (kalau Anda ingin strict)
    # ids = sorted(names.keys())
    # if ids != list(range(len(ids))):
    #     raise ValueError(f"[ERROR] index class tidak berurutan 0..N-1: {ids}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path root dataset YOLO (yang berisi images/ labels/ data.yaml). Default: <project_root>/Data/Datasets/DorisjuarsaDatasetYoloBaseSize",
    )
    parser.add_argument(
        "--sample",
        type=int,
    ...
    default_dataset = project_root / "Data" / "Datasets" / "DorisjuarsaDatasetYoloBaseSize"
